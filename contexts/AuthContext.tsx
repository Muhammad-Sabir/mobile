...
export class AuthProvider extends Component<AuthProviderProps, AuthProviderState> {
  ...
  login = async (phone: string, password: string, role: Role, callback: (success: boolean, error: any, message?: string) => void) => {
    try {
      const response = await fetch("https://scorpion-expert-marmoset.ngrok-free.app/api/user/token/", {
        method: "POST",
        headers: {
      ...
      if (responseData.hasOwnProperty("token")) {
        const userDetails = await fetch("https://scorpion-expert-marmoset.ngrok-free.app/api/user/me", {
          method: "GET",
          headers: {
  ...
  register = async (name: string, email: string, phone: string, password: string, role: Role, callback: (success: boolean, error: any) => void) => {
    ...
    try {
      const response = await fetch("https://scorpion-expert-marmoset.ngrok-free.app/api/user/create/", {
        method: "POST",
        headers: {
          "Accept": "application/json",
          "Content-Type": "application/json"
